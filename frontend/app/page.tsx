  ...
  return (
    <div className="min-h-screen bg-[#070a12] text-slate-100 flex flex-col font-sans selection:bg-teal-500 selection:text-slate-950">
      ...
      <main className="flex-1 max-w-[1650px] w-full mx-auto p-4 md:p-6 space-y-5">
        ...
        {/* Section 29: Scientific Transparency Category Badges */}
        <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-3.5 flex flex-wrap items-center justify-between gap-3 backdrop-blur-md shadow-xl text-xs">
          <div className="flex items-center gap-2">
            ...
            <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 font-semibold" title="Derived directly from satellite sensors (Landsat 8 TIRS, Sentinel-2)">
              Observed (Satellite LST)
            </span>
            <span className="px-2.5 py-0.5 rounded-full bg-blue-500/10 text-blue-400 border border-blue-500/30 font-semibold" title="Calculated from spatial averages and percentile rules">
              Derived (Anomaly & Risk)
            </span>
            <span className="px-2.5 py-0.5 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/30 font-semibold" title="Getis-Ord Gi* spatial autocorrelation tests">
              Statistical (Getis-Ord Gi*)
            </span>
            <span className="px-2.5 py-0.5 rounded-full bg-slate-800 text-slate-400 border border-slate-700 font-normal">
              Modeled / Simulated (Phase 3/4)
            </span>
          </div>
        ...
        {/* Header Control Toolbar with Requirement 15 Layer Selector */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-sm font-bold text-slate-100 flex items-center gap-2">
                Pune, MH (India)
                <span className="px-2 py-0.5 text-[10px] font-semibold bg-rose-500/10 text-rose-400 border border-rose-500/30 rounded-full">
                  Phase 2 Urban Heat Intelligence
                </span>
              </div>
            </div>
          </div>

          {/* Section 15: Layer selector UI */}
        ...
        {/* Section 19: Dynamic Dashboard Summary Cards */}
        ...
        {/* Core Map & Microclimate Inspector Grid */}
      ...
      <footer className="border-t border-slate-800/80 py-4 px-6 text-center text-xs text-slate-500 flex flex-wrap justify-between items-center max-w-[1650px] mx-auto w-full">
        <div>
          <strong className="text-slate-400">URBAN-COOL AI</strong> — Phase 2 Urban Heat Hotspot & Stress Mapping
        </div>
        <div className="flex gap-4 font-mono text-[11px]">
          <span>Frontend: Next.js 14</span>
          <span>Backend: FastAPI</span>
          <span>CRS: EPSG:4326 / EPSG:32643</span>
          <span>Hotspots: Getis-Ord Gi*</span>
        </div>
      </footer>
    </div>
  );
